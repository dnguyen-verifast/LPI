//==============================================================================
// lpi_qchannel_slave_monitor.sv
// Q-Channel SLAVE-side monitor : decodes the interface state every clock and
// publishes a transaction on each state change. Lives in / belongs to the
// slave agent.
//==============================================================================
class lpi_qchannel_slave_monitor extends uvm_monitor;
  `uvm_component_utils(lpi_qchannel_slave_monitor)
  ...
  task run_phase(uvm_phase phase);
    ...
    forever begin
      ...
      if (curr != prev) begin
        lpi_qchannel_item it = lpi_qchannel_item::type_id::create("slv_mon_it");
        it.prev_state = prev;
        it.curr_state = curr;
